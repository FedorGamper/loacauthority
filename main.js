...
global.mongo = require("./model/mongodb");
global.loac = require("loacprotocol").init("p192");
global.ia = new loac.IdentityAuthority(24 * 3600 * 7, secret.ia.sk);
global.pa = new loac.PermissionAuthority(secret.pa.sk);

//const User = require("./model/User");
//const Permission = require("./model/Permission");

//initialise new users
/*
(async () => {
    await mongo.addUser(new User(false, 'batman', 'test user', '$2b$10$uQu5POudgz/PilxYAldzLuUlUmhBOxYw8nxhIy/eGWNfmJnFo6O6y'))
        .then((user) => console.log("user added: " + user))
            .catch(err => console.log(err));
    await mongo.addUser(new User(true, 'admin', 'test admin', '$2b$10$uQu5POudgz/PilxYAldzLuUlUmhBOxYw8nxhIy/eGWNfmJnFo6O6y'))
        .then((user) => console.log("user added: " + user))
            .catch(err => console.log(err));
    await mongo.addPermission("batman" ,new Permission("Fedors Raspi",
        "fedorspi", "von Fedor",
...
function auth(req, res, next) {
    ...
    if (authBase64 !== undefined) {
        // basic out header is formatted like this example: "Basic YWRtaW46YWRtaQ=="
        const auth = new Buffer(authBase64.split(" ").pop(), 'base64').toString();

        var [username, pw] = auth.split(":");
        // get user record form database
        mongo.findUser(username)
            .then(user => {
                let pwHashed;
                if(user !== null) {
                    pwHashed = user.password;
                }
                if (pwHashed && pw != null && bcrypt.compareSync(pw, pwHashed)) {
                    console.log("correct password");
                    //save the model result for to limit further searches
                    req.record = user;
