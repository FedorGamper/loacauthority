...
try {
    db.addUser(new User(false, 'batman', 'test user', '$2b$10$uQu5POudgz/PilxYAldzLuUlUmhBOxYw8nxhIy/eGWNfmJnFo6O6y')); //pw = password

    let tmpPermission = new Permission("Fedors Raspi",
        "fedorspi", "von Fedor",
    ...
    db.addPermission("batman", tmpPermission);

    db.addUser(new User(true, 'admin', 'test admin', '$2b$10$uQu5POudgz/PilxYAldzLuUlUmhBOxYw8nxhIy/eGWNfmJnFo6O6y'));
    db.addCert("admin", "something");
} catch (e) {
    console.log(e);
}
...
function auth(req, res, next) {
    ...
    if (authBase64 !== undefined) {
        ...
        //var [username, pw] = authBase64.split(":");
        // get user record form database
        var record = db.findUserByName(username);
        var pwHashed = record.password;
    }
    // check if hashed pw was found in model and if the pw field in authentication is not empty
